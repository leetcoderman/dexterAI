...
function runMigrations(database: DatabaseType.Database): void {
    const row = database.prepare("SELECT value FROM metadata WHERE key = 'schema_version'").get() as { value: string } | undefined;
    ...
    for (const migration of migrations) {
        if (migration.version > currentVersion) {
            ...
            migration.up();
            database.prepare("INSERT OR REPLACE INTO metadata (key, value) VALUES ('schema_version', ?)")
