  ...
  ipcMain.handle('chat:send', async (event, request: {
    ...
    try {
      ...
      try {
        ...
        const conv = db.prepare('SELECT settings_json FROM conversations WHERE id = ?').get(request.conversationId) as { settings_json: string } | undefined
        const settings = conv?.settings_json ? JSON.parse(conv.settings_json) : {}
        const memoryEnabled = settings.memoryEnabled !== false

        if (memoryEnabled) {
          const memories = db.prepare('SELECT content, is_pinned FROM memories ORDER BY is_pinned DESC, updated_at DESC').all() as { content: string; is_pinned: number }[]
          if (memories.length > 0) {
            ...
            if (sysIdx !== -1) {
              messages[sysIdx] = { ...messages[sysIdx], content: messages[sysIdx].content + memoryBlock }
            } else {
      ...
      try {
        const db = getDatabase()
        const conv = db.prepare('SELECT title FROM conversations WHERE id = ?').get(request.conversationId) as { title: string | null } | undefined
        if (conv && !conv.title) {
          const firstUserMsg = request.messages.find((m) => m.role === 'user')
          if (firstUserMsg) {
            autoTitle(event.sender, request.conversationId, request.providerId, request.modelId, firstUserMsg.content)
          }
        }
      } catch (e) {
