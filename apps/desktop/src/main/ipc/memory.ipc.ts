...
export function registerMemoryHandlers() {
  ...
  ipcMain.handle('memory:togglePin', async (_, id: string) => {
    try {
      const db = getDatabase()
      db.prepare(
        "UPDATE memories SET is_pinned = CASE WHEN is_pinned = 1 THEN 0 ELSE 1 END, updated_at = datetime('now') WHERE id = ?"
      ).run(id)
      return { success: true }
    } catch (err: any) {
      return { success: false, error: err.message }
  ...
  ipcMain.handle(
    'memory:extract',
    async (_, conversationId: string, providerId: string, modelId: string) => {
