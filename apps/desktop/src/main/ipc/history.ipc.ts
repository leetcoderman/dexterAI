...
export function registerHistoryHandlers() {
    ...
    ipcMain.handle('history:exportAsCSV', async (_, modelId: string) => {
        try {
            const db = getDatabase();
            const rows = db.prepare('SELECT * FROM test_runs WHERE model_id = ? ORDER BY ran_at DESC').all(modelId) as TestRunRecord[];
            const header = 'id,model_id,provider_id,category,ran_at,ttft,prompt_tokens,completion_tokens,error';
            const lines = rows.map(r => {
                let metrics: any = {};
                try { metrics = JSON.parse(r.metrics_json); } catch { /* ignore */ }
                return [
                    r.id,
                    r.model_id,
                    r.provider_id,
