...
export class GoogleAdapter extends BaseProviderAdapter {
  ...
  private mapMessagesToGeminiContents(messages: any[]): {
    contents: any[]
  ...
  async execute(
    ...
    try {
      const { contents, systemInstruction } = this.mapMessagesToGeminiContents(
        params.messages || []
      )

      if (contents.length === 0 && params.prompt) {
        const rawPrompt =
          typeof params.prompt === 'string' ? params.prompt.trim() : String(params.prompt).trim()
