...
export class DeepgramAdapter extends BaseProviderAdapter {
    ...
    async verify(credentials: ProviderCredentials): Promise<VerifyResult> {
        try {
            ...
            if (!result || result.projects.length === 0) {
                return { success: false, error: { code: 'INVALID_KEY', message: 'Valid key but no active projects', isRetryable: false } };
            }
            return { success: true };
        } catch (e: any) {
            return { success: false, error: { code: 'INVALID_KEY', message: `Deepgram API Error: ${e.message}`, isRetryable: false } };
        }
    }
    ...
    async execute(request: TestRequest, credentials: ProviderCredentials, emitter: IPCEmitter): Promise<void> {
        ...
        try {
            ...
            if (!params.filePath) {
                throw new Error('Live Microphone streaming is pending Frontend implementation. Please upload an audio file instead.');
            }
    ...
    private async executeTTS(request: TestRequest, credentials: ProviderCredentials, emitter: IPCEmitter): Promise<void> {
        const { modelId, params } = request;
        const startTime = Date.now();
            ...
            const audioData = await new Promise<Buffer>((resolve, reject) => {
                ...
                const req = https.request(url, {
                    ...
                    if (res.statusCode && res.statusCode >= 400) {
                        ...
                        res.on('end', () => reject(new Error(`Deepgram TTS API error ${res.statusCode}: ${errorBody}`)));
                        return;
                    }
                    ...
                    res.on('data', (chunk: Buffer) => {
                        if (!firstByte) {
                            ...
                            emitter.emit('test:chunk', { requestId: request.requestId, text: `[TTFB: ${ttfb}ms] ` });
                        }
                        chunks.push(chunk);
                    });
