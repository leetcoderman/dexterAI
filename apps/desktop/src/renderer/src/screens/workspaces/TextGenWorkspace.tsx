...
export default function TextGenWorkspace({
  providerId,
  modelId
}: {
  providerId: string
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: `[Error starting stream: ${e.message}]` }
      ])
    }
  }
    ...
    if (params.systemPrompt !== undefined) {
      setMessages((prev) => {
        ...
        if (copy[0]?.role === 'system') {
          copy[0] = { ...copy[0], content: params.systemPrompt }
        } else {
          copy.unshift({ role: 'system', content: params.systemPrompt })
        }
        return copy
      })
    }
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 min-h-[200px]">
        {messages.filter((m) => m.role !== 'system').length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 space-y-4 py-16">
            ...
            <p>
              Enter a prompt below to start testing <span className="font-medium">{modelId}</span>.
            </p>
          </div>
        ) : (
          messages.map((m, idx) => {
      ...
      <div className="p-4 bg-background border-t border-border flex flex-col space-y-3 shrink-0">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2 flex flex-col justify-center">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="jsonMode" className="text-xs font-medium text-gray-600">
                      Force JSON Mode
                    </label>
                  </div>
                  {!jsonMode && (
                    <select
                      ...
                      className="w-full px-2 py-1 border border-border rounded text-xs bg-background focus:outline-none"
                    >
        ...
        <div className="relative w-full flex items-end space-x-2">
          ...
          {isStreaming ? (
            <button
              onClick={handleStop}
              className="w-12 h-[50px] flex items-center justify-center shrink-0 border border-red-500/20 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500/20 transition-colors"
              title="Stop"
            >
            ...
            <button
              onClick={handleRun}
              disabled={!input.trim()}
              className="w-12 h-[50px] flex items-center justify-center shrink-0 bg-primary text-white rounded-xl hover:bg-primary/90 transition-colors shadow-sm disabled:opacity-50"
              title="Send"
            >
              {isPending ? (
