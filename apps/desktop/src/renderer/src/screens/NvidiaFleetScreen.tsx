...
const MATRIX_DATA = [
  ...
  },
  {
    ...
    params: '671B (MoE)',
    context: '128K',
    strength: 'Agentic CLI',
    score: 5
  },
  {
    name: 'Nemotron-3 Nano',
    params: '30B (3.2B Active)',
    context: '1M',
    strength: 'Multi-agent Swarms',
    score: 4
  },
  {
    name: 'Cosmos Reason2',
    ...
    <div className="h-full overflow-y-auto bg-[#0a0a0b] text-white selection:bg-[#76b900]/30 font-sans">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8 py-16">
        ...
        <header className="mb-20 text-center animate-fade-in-up">
          ...
          <p className="max-w-3xl mx-auto text-xl text-gray-400 font-medium leading-relaxed mb-8">
            Transitioning from monolithic models to a symphony of hyper-specialized agentic
            microservices. Optimized for{' '}
            <span className="text-white font-bold">Blackwell Architecture</span> and{' '}
            <span className="text-white font-bold">NIM Microservices</span>.
          </p>
          <button
            onClick={() => navigate('/catalogue?provider=nvidia_nim')}
            className="inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-[#76b900] text-black font-black uppercase tracking-wider hover:bg-[#88cc00] transition-colors shadow-[0_0_20px_rgba(118,185,0,0.4)]"
          >
            Explore NVIDIA Models in Catalogue
          ...
          <section className="space-y-12 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
              {[
                ...
                { label: 'Latency', value: '< 1.0s', desc: 'First Token Average', icon: Clock },
                {
                  label: 'Architecture',
                  value: 'MoE-Native',
                  desc: 'Smarter Efficiency',
                  icon: Cpu
                }
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="p-8 rounded-[32px] bg-white/5 border border-white/10 hover:border-[#76b900]/40 transition-all group"
                >
                  <stat.icon className="w-8 h-8 text-[#76b900] mb-6 group-hover:scale-110 transition-transform" />
                  <div className="text-4xl font-black mb-1">{stat.value}</div>
                  <div className="text-xs font-black uppercase text-gray-500 tracking-widest">
                    {stat.label}
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {MODELS.map((model) => (
                <div
                  key={model.id}
                  className="group relative p-10 rounded-[40px] bg-white/5 border border-white/10 overflow-hidden hover:bg-white/[0.08] transition-all"
                >
                  <div
                    className={cn(
                      'absolute top-0 right-0 w-64 h-64 blur-[100px] opacity-10 group-hover:opacity-20 transition-opacity bg-gradient-to-br',
                      model.color
                    )}
                  />
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className="text-4xl font-black mb-4 group-hover:text-[#76b900] transition-colors">
                      {model.name}
                    </h3>
                    ...
                    <div className="grid grid-cols-3 gap-6 py-6 border-t border-white/10">
                      {Object.entries(model.metrics).map(([key, val]) => (
                        <div key={key}>
                          <div className="text-[10px] font-black uppercase text-gray-500 tracking-widest mb-2">
                            {key}
                          </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-white/10 bg-white/[0.02]">
                      <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-gray-500">
                        Model Name
                      </th>
                      <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-gray-500">
                        Params
                  ...
                  <tbody>
                    {MATRIX_DATA.map((row, i) => (
                      ...
                      >
                        <td className="px-8 py-6 font-bold group-hover:text-[#76b900]">
                          {row.name}
                        </td>
                            ...
                            {[...Array(5)].map((_, j) => (
                              <div
                                ...
                                className={cn(
                                  ...
                                  j < row.score ? 'bg-amber-400' : 'bg-white/10'
                                )}
                              />
                            ))}
                          </div>
              ...
              {!recommendations ? (
                <div className="relative z-10 text-center space-y-12">
                  <div className="space-y-4">
                    ...
                    <p className="text-gray-400">
                      Identify the optimal engine for your current directive.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      {
                        id: 'code',
                        label: 'Full Repo Refactor',
                        icon: Code,
                        desc: 'Highest coding proficiency'
                      },
                      {
                        id: 'speed',
                        label: 'Agentic Swarms',
                        icon: Zap,
                        desc: 'Max throughput & sub-second latency'
                      },
                      {
                        id: 'reasoning',
                        label: 'Advanced Reasoning',
                        icon: Brain,
                        desc: 'Graduate-level problem solving'
                      },
                      {
                        id: 'robotics',
                        label: 'Robotics & Vision',
                        icon: Target,
                        desc: 'Physical world intelligence'
                      }
                    ].map((ans) => (
                      <button
                        key={ans.id}
                ...
                <div className="relative z-10 text-center animate-fade-in-up">
                  ...
                  <div className="flex justify-center gap-4">
                    ...
                    <button className="px-8 py-4 rounded-2xl bg-[#76b900] text-black text-sm font-bold shadow-glow hover:scale-[1.05] transition-all flex items-center gap-2">
                      Launch Model
