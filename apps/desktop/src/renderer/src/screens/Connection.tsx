...
export default function Connection() {
  ...
  const providerLabel = providerId ? providerId.charAt(0).toUpperCase() + providerId.slice(1) : ''
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Keys are stored in your OS keychain, never in the cloud.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {detectProviderFromKey(apiKey) && detectProviderFromKey(apiKey) !== providerId && (
            <div className="flex items-start space-x-2 text-yellow-600 bg-yellow-50 dark:text-yellow-500 dark:bg-yellow-500/10 p-3 rounded-md text-sm font-medium">
              ...
              {detectProviderFromKey(apiKey) === 'nvidia_nim' ? (
                <p>
                  This looks like an NVIDIA NIM key (nvapi-). To use NVIDIA NIM, switch to the
                  NVIDIA NIM connector in the sidebar.
                </p>
              ) : (
                <p>
                  This key format ({detectProviderFromKey(apiKey)}) doesn't match the current
                  provider ({providerId}).
                </p>
              )}
            </div>
          ...
          <button
            ...
            disabled={
              isVerifying ||
              !apiKey.trim() ||
              (detectProviderFromKey(apiKey) !== null &&
                detectProviderFromKey(apiKey) !== providerId)
            }
