...
export default function TemplateManager({ category, onLoadTemplate, getCurrentParams }: TemplateManagerProps) {
        ...
        <div className="flex flex-col mb-4 space-y-2 relative z-10">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                    {justSaved && <span className="text-emerald-500 text-xs flex items-center"><CheckCircle2 className="w-3 h-3 mr-1" /> Saved</span>}
            ...
            {templates.length > 0 && (
                <div className="flex overflow-x-auto space-x-2 pb-1 scrollbar-hide">
                    {templates.slice(0, 5).map(t => (
                        <div key={t.id} className="flex-shrink-0 flex items-center bg-black/5 dark:bg-white/5 border border-border rounded-full px-3 py-1 group">
                            <button
