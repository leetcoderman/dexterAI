...
export default function EditableTitle({
  ...
  return (
    <div
      className={cn(
        'inline-flex items-center gap-1.5 min-w-0 group/title cursor-text w-fit max-w-full',
        className
      )}
      onClick={(e) => {
        if (clickToEdit) {
    ...
    >
      <span className={cn('truncate', textClassName)}>{value || 'Untitled Conversation'}</span>
      {showEditIcon && (
        <button
          onClick={(e) => {
            e.stopPropagation()
